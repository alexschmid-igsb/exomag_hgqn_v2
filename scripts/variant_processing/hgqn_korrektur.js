...
const transcript = 'mane_select'

const VVStore = require('../VVStore/VVStore')

const VV_GRCh38 = VVStore.loadStore('HGQN_gDNA','GRCh38',transcript)

// const VV_GRCh37 = VVStore.loadStore('HGQN_gDNA','GRCh37',transcript)
// const VV_GRCh38 = VVStore.loadStore('HGQN_gDNA','GRCh38',transcript)
...
async function getVVResult(build,inputString) {
    ...
    /*
    if(build === 'GRCh37') {
        result.vvResponse = VV_GRCh37.get(inputString)
    } else if (build === 'GRCh38') {
        result.vvResponse = VV_GRCh38.get(inputString)
    }
    */
    result.vvResponse = VV_GRCh38.get(inputString)

    if(result.vvResponse == null) {
        result.error = {
            state: 'ERROR',
            message: `Missing VV response: ${inputString}`,
        }
        return result
    }
    
    if(result.vvResponse.__STORE_ERROR__ != null) {
        result.error = {
            state: 'ERROR',
            message: `Error in VV store for input: ['${build}', '${inputString}', '${transcript}'].`,
            cause: {
                name: 'ERROR',
                message: result.vvResponse.__STORE_ERROR__
            }
        }
        return result
    }
    ...
    if(result.vvResponse != null && result.vvResponse.flag !== 'gene_variant') {
        result.error = {
            state: 'ERROR',
            message: `VariantValidator returned error flag: ${inputString}`
        }
        return result
    }
    ...
    for(let [vvKey,vvEntry] of Object.entries(result.vvResponse)) {
        if(vvEntry.primary_assembly_loci != null) {
            ...
            if(result.parsedLoci == null) {
                ...
                // es gibt breits ein geparsten loci. der soeben geparste muss identisch sein, alles andere ist als fehler zu werten
                let isEqual = lodash.isEqual(parsed, result.parsedLoci)
                if(isEqual === false) {
                    // console.log("eins")
                    // console.log(parsed)
                    // console.log("zwei")
                    // console.log(result.parsedLoci)
                    result.error = {
                        state: 'ERROR',
...
async function main() {
    ...
    // console.log(VV_GRCh37)
    // console.log(VV_GRCh38)
    ...
    for(let [i,dbVariant] of res.data.entries()) {
        ...
        let hasErrors = false
        let istVertauscht = false

        // get vv output
        let result_GRCh37 = await getVVResult('GRCh37',dbVariant.GRCh37.gDNA)
        let result_GRCh38 = await getVVResult('GRCh38',dbVariant.GRCh38.gDNA)

        if(result_GRCh37.error.state !== 'VALID' || result_GRCh38.error.state !== 'VALID') {
            ...
            if(result_GRCh37.error.state !== 'VALID') {
                ...
                if(result_GRCh37.error.cause != null) out += `     caused by: ${result_GRCh37.error.cause.message}\n`
                hasErrors = true
            }
            if(result_GRCh38.error.state !== 'VALID') {
                ...
                if(result_GRCh38.error.cause != null) out += `     caused by: ${result_GRCh38.error.cause.message}\n`
                hasErrors = true
            }
        ...
        if(hasErrors === true && istVertauscht === false) {
            // nach vertauschung über regex suchen
            let chr_GRCh37 = dbVariant.GRCh37.gDNA.substring(0,dbVariant.GRCh37.gDNA.indexOf(':'))
            let chr_GRCh38 = dbVariant.GRCh38.gDNA.substring(0,dbVariant.GRCh38.gDNA.indexOf(':'))

            const pattern = /(NC_\d{6})\.(\d{1,2})/

            let re_chr_GRCh37 = chr_GRCh37.match(pattern)
            let re_chr_GRCh38 = chr_GRCh38.match(pattern)

            if(re_chr_GRCh37[1] === re_chr_GRCh38[1] && parseInt(re_chr_GRCh37[2]) > parseInt(re_chr_GRCh38[2])) {
                hasErrors = true
                out += '\n'
                out += '     VERTAUSCHUNG GEFUNDEN (chr vergleich)\n'
                istVertauscht = true
            }
        }
        ...
        // vertauschung korrigieren
        if(istVertauscht === true) {
    ...
    // console.log(Array.from(bla.values()).sort())
